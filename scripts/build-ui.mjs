...
const ELECTRON_SHIMS = [
  'electron-dialog.js', // 修 Electron 無 window.prompt → 建立變數無視窗
  'electron-clipboard.js', // 修 Windows 剪貼簿焦點問題 → 滑鼠複製貼上失效（#8882）
];
...
function patchIndexHtml() {
  ...
  // Electron 專屬 shim 注入：
  //   electron-dialog.js    修 window.prompt 不存在（Blockly 建立變數無視窗）
  //   electron-clipboard.js 修 Windows 剪貼簿焦點問題（滑鼠複製貼上失效 #8882）
  ...
  if (!html.includes(blocklyTag)) {
    throw new Error('index.html 找不到 javascript_compressed.js 引用，無法注入 Electron shim，請更新 build-ui.mjs');
  }
  const shimTags = ELECTRON_SHIMS.map((shim) => {
    copy(path.join(SHIM_DIR, shim), path.join(OUT, shim));
    return `<script src="${shim}?rev=${REV}"></script>`;
  });
  html = html.replace(blocklyTag, [blocklyTag, ...shimTags].join('\n  '));

  fs.writeFileSync(path.join(OUT, 'index.html'), html);
  log('index.html 處理完成（安裝版模式 + Electron shims + rev=' + REV + '）');
}
